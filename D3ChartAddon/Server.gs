 ...
 */
var DATA_ALIAS = 'Google Spreadsheet';
var ADDON_NAME = "D3 Charting";
var SIDEBAR_TITLE = 'Import and Chart';
...
function showSidebar() {
  ...
  template.isAuthorized = true; //service.hasAccess();
 ...
 */
function runChart(reportId) {
  myLogger_('runChart');
  var template = HtmlService.createTemplateFromFile('d3-dialog');
  ...
  var sheetDataArr = getSpreadsheetRows(spreadsheet,sheetName);
  var sheetDataArrLen = sheetDataArr.length;
  
  var hdrArr = ["period","user","value"]
  var dataArr = [];
  var rowObj;
  for (var i=1;i<sheetDataArrLen;i++) { // skip header
    var sheetDataRowObj = sheetDataArr[i];
    rowObj = {};
    for (var col in sheetDataRowObj) {
      switch (col) {
        case "_index":
          break;
        case "count":
          rowObj.value = sheetDataRowObj[col]; // re-label count to value
          break;
        default:
          rowObj[col] = sheetDataRowObj[col];
          break;
      }
    }
    dataArr.push(rowObj);
  }
  //myLogger_(JSON.stringify(dataArr));
  configObj.data = dataArr;
  
  template.configObj = configObj;
  
  var html = template.evaluate().setSandboxMode(HtmlService.SandboxMode.IFRAME);  // IFrame required to support D3
...
function runImport(reportId) {
  ...
  try {
    var page;
    //do {
      myLogger_('columnIds '+JSON.stringify(columnIds));
